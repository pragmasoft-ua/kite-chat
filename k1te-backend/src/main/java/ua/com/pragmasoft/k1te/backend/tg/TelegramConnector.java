...
public class TelegramConnector implements Connector, Closeable {
  ...
  private String onSwitchConnection(String channelName, String memberId, String newConnection) {
    ...
    // TODO: 17.11.2023 Fix problem when sending BinaryMessage to TgConnector.dispatch() it fails to
    // deliver message, due incorrect file url generated by tg
    ...
    for (int i = historyMessages.size() - 1; i >= 0; i--) {
      ...
      if (payload instanceof PlaintextMessage plaintextMessage) {
        ...
        payload =
            new PlaintextMessage(prefix + plaintextMessage.text(), plaintextMessage.messageId());
      }
      var context =
          RoutingContext.create()
              .withOriginConnection(newConnection)
